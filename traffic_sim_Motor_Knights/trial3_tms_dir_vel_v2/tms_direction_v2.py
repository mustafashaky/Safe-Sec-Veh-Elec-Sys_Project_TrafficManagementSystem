...
MAX_CARS_PER_DIR = 4         # cap to avoid infinite buildup
TOTAL_CARS = 100             # total cars per experiment run
LIAR_PROBABILITY = 0.25       # some cars lie about intent
USE_LIARS = False               # global switch: if False, no new liars are spawned
...
HEADWAY_BY_INTENT = {
    ...
    "right":    MIN_HEADWAY, #0.9
    "left":     MIN_HEADWAY, #0.3
}
...
def main():
    ...
    if crash_records:
        try:
            ...
            df.to_csv('tms_crash_log.csv', index=False)
            print(f'Saved {len(crash_records)} crash records to tms_crash_log.csv')
        except Exception as e:
            # Fallback: write a very simple CSV if pandas is unavailable.
            import csv
            keys = crash_records[0].keys()
            with open('tms_crash_log.csv', 'w', newline='') as f:
                writer = csv.DictWriter(f, fieldnames=keys)
                writer.writeheader()
                writer.writerows(crash_records)
            print(f'Saved {len(crash_records)} crash records to tms_crash_log.csv (csv module fallback)')
